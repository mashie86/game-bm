...
(function () {
  ...
  function tunjukKeputusan(betul, jumlah, hasil) {
    ...
    document.getElementById('keputusan-bintang').textContent = '⭐'.repeat(bintang) + '☆'.repeat(3 - bintang);

    let emoji = '🎉', tajuk = 'Syabas!', mesej = 'Teruskan usaha, kamu semakin hebat!';
    if (bintang === 3) { emoji = '🏆'; tajuk = 'Cemerlang!'; mesej = 'Semua jawapan betul! Kamu memang hebat!'; }
    else if (bintang === 0) { emoji = '💪'; tajuk = 'Jangan Putus Asa!'; mesej = 'Cuba lagi sekali untuk buka level seterusnya.'; }
    else if (hasil.levelSeterusnyaDibuka) { mesej = 'Bagus! Level seterusnya telah dibuka.'; }
